...
MediaWikiSetting.prototype.getObj=function(path,method){
    ...
    return {
        ...
        port:u.port || 80,
        path:this.basepath+u.path,
        method:method|| "GET",
    };
};
function Bot(argv,setting){
    this.argv=argv;
...
DBManager.prototype.savePage=function(pageobj,callback){
    //dummy!!!
    callback();
}
...
//具体的操作
//ページの情報を保存（怪しい）
MongoDBManager.prototype.savePage=function(doc,callback){
    var bot=this.bot;
    this.collection("pages",function(coll){
        coll.update({title:doc.title},doc,{
            safe:true,
            upsert:true,
        },function(err){
            if(err){
...
//保存されたページをアレする
MongoDBManager.prototype.iteratePages=function(query,callback){
    var bot=this.bot;
    this.collection("pages",function(coll){
...
exports.Bot=Bot;
exports.MediaWikiSetting=MediaWikiSetting;
